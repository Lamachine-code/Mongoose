...
void skipWhitespaceAndComments(Lexer *lexer) {
  for (;;) {
    ...
    case '#':
      ...
      break;
    case '/':
      // Vox style comments start with '//'; a lone '/' is the division operator
      if (lexer->current[1] != '/') {
        return;
      }
      while (peek(lexer) != '\n' && peek(lexer) != '\0') {
...
static TokenType checkKeyword(Lexer *lexer, int startOffset, int length,
...
static Token identifierOrKeyword(Lexer *lexer) {
  ...
  switch (firstChar) {
  case 'a':
    type = checkKeyword(lexer, 0, 3, "and", TOKEN_AND);
    break;
  case 'l':
    type = checkKeyword(lexer, 0, 3, "let", TOKEN_LET);
    break;
  case 'f':
    // 'f' can be "function" or "false"
    // Hint: check the second character to branch or chain checkKeyword calls
    if (computeLength(lexer) >
        1) { // Ensure the lexeme has at least two characters
      if (lexer->start[1] == 'u')
        type = checkKeyword(lexer, 0, 8, "function", TOKEN_FUNCTION);
      else if (lexer->start[1] == 'a')
        type = checkKeyword(lexer, 0, 5, "false", TOKEN_FALSE);
    }
    break;
  case 'i':
    type = checkKeyword(lexer, 0, 2, "if", TOKEN_IF);
    break;
  case 't':
    if (computeLength(lexer) > 1) {
      if (lexer->start[1] == 'h')
        type = checkKeyword(lexer, 0, 4, "then", TOKEN_THEN);
      else if (lexer->start[1] == 'r')
        type = checkKeyword(lexer, 0, 4, "true", TOKEN_TRUE);
    }
    break;
  case 'e':
    type = checkKeyword(lexer, 0, 4, "else", TOKEN_ELSE);
    break;
  case 's':
    type = checkKeyword(lexer, 0, 4, "stop", TOKEN_STOP);
    break;
  case 'o':
    type = checkKeyword(lexer, 0, 2, "or", TOKEN_OR);
    break;
  case 'n':
    type = checkKeyword(lexer, 0, 3, "not", TOKEN_NOT);
    break;
  }
...
Token nextToken(Lexer *lexer) {
  ...
  char c =
      advance(lexer); // After this line, "lexer->current" advances to the next
                      // character. "c" contains the consumed character.
  // Example: in "let c =3", if "c = advance(lexer)" returns "=", then after
  // execution "lexer->current" will point at "3".

  // 3. Token recognition
  if (isalpha(c))
    return identifierOrKeyword(lexer);
  if (isdigit(c))
    return number(lexer);
  ...
  case '=':
    return match(lexer, '=') ? makeToken(lexer, TOKEN_EQUAL)
                             : makeToken(lexer, TOKEN_ASSIGN);
  case '!':
    if (match(lexer, '='))
      return makeToken(lexer, TOKEN_NOTEQUAL);
    break;
  case '<':
    return match(lexer, '=') ? makeToken(lexer, TOKEN_LTEQ)
                             : makeToken(lexer, TOKEN_LT);
  case '>':
    return match(lexer, '=') ? makeToken(lexer, TOKEN_GTEQ)
                             : makeToken(lexer, TOKEN_GT);
  }
