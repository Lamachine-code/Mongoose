...
typedef enum {
    ...
    TOKEN_ELSE, TOKEN_STOP, TOKEN_TRUE, TOKEN_FALSE,
...
typedef struct {
    const char* source;
    const char* start;      // Pointer to the start of the token currently being processed/analyzed in the source string
    const char* current;
    int line;
